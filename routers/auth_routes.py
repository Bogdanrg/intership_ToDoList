...
from schemas import SignUpModel, SignInModel, TokenPairModel
...
async def register_user(user: SignUpModel) -> Any:
    async with AsyncSession(engine) as session, session.begin():
        await UserRepository.insert_one(session,
...
@auth_router.put("/change-username/")
async def update_username(username: Annotated[str | None, Query(max_length=50)] = "new_username") -> Any:
    async with AsyncSession(engine) as session, session.begin():
...
async def login_user(user: SignInModel) -> Any:
    async with AsyncSession(engine) as session, session.begin():
        is_valid_user = await JWTService.check_credentials(session, user.username, user.password)
