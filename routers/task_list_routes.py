...
@task_list_router.post("/", response_model=TaskListModel)
async def create_list(request: Request, task_list: TaskListModel) -> Any:
    task_list_obj = await TaskListRepository.get_task_list_by_name_and_user(
        request.state.session, task_list.name, request.state.user
    )
...
@task_list_router.get("/", response_model=List[TaskModel])
async def get_task_list(request: Request, name: str = Body(embed=True)):
    task_list_obj = await TaskListRepository.get_task_list_by_name_and_user(
        request.state.session, name, request.state.user
    )
    ...
    tasks = await TaskListRepository.get_task_list(request.state.session, task_list_obj)
    return tasks


@task_list_router.post("/add/", response_model=TaskModel)
async def add_task(
